...
class InLinePos(nn.Module):
    ...
    def get_a_poser(self, line_lens, task_name) -> torch.Tensor:
        ...
        if task_name.endswith('_r2l'):
            ...
            poser=self.pe_r[tmp:tmp + 1]
            
        if '_' not in task_name:
            poser=torch.zeros((2*len(line_lens), self.d_model))
            for idx, n in enumerate(line_lens):
                assert n%2==0 ,'unexpected odd number when posing'
                half_n = n // 2
                # Fill first half with forward positions and add last position encoding from it
                forward_positions = self.pe[:half_n]
                forward_last_position = forward_positions[-1].unsqueeze(0)
                # Fill second half with reversed backward positions and add last position encoding from it
                backward_positions = self.pe_r[:half_n].flip([0])
                backward_last_position = backward_positions[0].unsqueeze(0)
                poser[2 * idx] = forward_last_position
                poser[2 * idx + 1] = backward_last_position
        return poser
