...
class Task_seqs():
    ...
    def to_dict(self):
        ...
        return {
            ...
            "li": self.li,
            "lens": self.lens,
            "attn_mask": self.attn_mask.tolist() if isinstance(self.attn_mask, torch.Tensor) else self.attn_mask,
            # "pos": self.pos,
            # "dest": self.dest
        }
    
    def visualize_to_file(self, filename):
        # Determine the maximum line index to organize input and tgt by lines
        max_line_index = max(self.li) if self.li else -1

        # Initialize buffers for inputs and targets organized by line
        lines_input = [[] for _ in range(max_line_index + 1)]
        lines_tgt = [[] for _ in range(max_line_index + 1)]

        # Distribute tokens to their respective lines based on li
        for idx, line_number in enumerate(self.li):
            lines_input[line_number].append(self.input[idx])
            lines_tgt[line_number].append(self.tgt[idx])

        with open(filename, 'w', encoding='utf-8') as f:
            # Write out each line showing input and target to the file
            for i in range(max_line_index + 1):
                f.write(f"Line {i}:\n")
                f.write(f"  Input: {' '.join(lines_input[i])}\n")
                f.write(f"  Target: {' '.join(lines_tgt[i])}\n")
                f.write("\n")  # Newline for better readability
    
    def visualize_attn_mask(self, filename='attn_mask.png',grid_size=10):
        n, m = self.attn_mask.shape
        height = n * grid_size
        width = m * grid_size
        ...
        # Draw black squares for True values in the attn_mask and add grid lines
        for i in range(n):
            for j in range(m):
                top_left = (j * grid_size, i * grid_size)
                bottom_right = ((j + 1) * grid_size, (i + 1) * grid_size)

                # Draw the square with color based on the attn_mask value
                if self.attn_mask[i, j]:  # If masked (True), draw a black square
                    cv2.rectangle(image, top_left, bottom_right, color=0, thickness=-1)
                
                # Draw the grid line (black color, 1 pixel thickness)
                cv2.rectangle(image, top_left, bottom_right, color=0, thickness=1)
    ...
    def save_to_json(self, filename):
        # Save the dictionary representation of the object to a JSON file
        with open(filename, 'w', encoding='utf-8') as f:
...
def make_seqs(tokens, inposer):
    ...
    task_seqs=edict(dict(zip(task_names,task_seqs)))
    ...
    task_seqs.plain_l2r.attn_mask=glue_line_triu([len(task_seqs.plain_l2r.input)], reversed=False, bidir=False, vis_other=True)
    task_seqs.plain_r2l.attn_mask=glue_line_triu([len(task_seqs.plain_r2l.input)], reversed=True, bidir=False, vis_other=True)
    task_seqs.sline_l2r.attn_mask=glue_line_triu([len(task_seqs.sline_l2r.input)], reversed=False, bidir=False, vis_other=True)
    task_seqs.sline_r2l.attn_mask=glue_line_triu([len(task_seqs.sline_r2l.input)], reversed=True, bidir=False, vis_other=True)
    task_seqs.mline_l2r.attn_mask=glue_line_triu(task_seqs.mline_l2r.lens, reversed=False, bidir=False, vis_other=True)
    task_seqs.mline_r2l.attn_mask=glue_line_triu(task_seqs.mline_r2l.lens, reversed=True, bidir=False, vis_other=True)
    task_seqs.bline.attn_mask=glue_line_triu(task_seqs.bline.lens, reversed=False, bidir=True, vis_other=True)
